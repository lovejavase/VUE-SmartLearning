		...
		request({
			...
			for (let j = 0; j < newsList.length; j++) {
				...
				request({
					...
					method: 'GET',
					data: {
						userId: newsList[j].newUser
					}
				}).then(user => {
					switch (newsList[j].newSort) {
						case '人工智能的技术与应用':
							news1.value.push({
								newsId: newsList[j].newId,
								newTitle: newsList[j].newTitle,
								userName: user.data.userNickName,
								newImg: newsList[j].newImg
							})
							break;
						case '人工智能的过去与发展':
							news2.value.push({
